CREATE function [dbo].[pl_GetSubjsGrid_tablo] (@DateFrom DateTime, @PlParamID int)
RETURNS @table TABLE (PlSubjID int, SubjName varchar(100), SpecializationID int,
					Specialization varchar(100), 
					Cabinet varchar(100), CabinetDay varchar(100),
...
BEGIN
  declare @SubjID int, @SubjName varchar(100), @SpecializationID int,
		  @Specialization varchar(100), @Medecin varchar(100),
		  @Cabinet varchar(100), @CabinetDay varchar(100),
...
*/   
		...
		select @CabinetDay=dbo.ParseLangString(PL_CABINETS.NAME,'rus')
		from PL_CABINETS_DAYS
		JOIN PL_CABINETS PL_CABINETS ON PL_CABINETS.PL_CABINETS_ID = PL_CABINETS_DAYS.PL_CABINETS_ID 
		where PL_CABINETS_DAYS.PL_DAY_ID=@PlDay
		  and PL_CABINETS_DAYS.PL_SUBJ_ID=@SubjID
		  
		  if(@ExclAllDay=0)
			insert into @table values (@SubjID, @SubjName, @SpecializationID, @Specialization,
								 @Cabinet, @CabinetDay, 
